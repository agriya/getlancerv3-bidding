export default function maExportToCsvButton($stateParams, Papa, notification, AdminDescription, entryFormatter, ReadQueries) {
    return {
        ...
        link: function(scope) {
            ...
            scope.has_export = exportView.fields()
                .length > 0;
                ...
                ReadQueries.getAll(exportView, -1, $stateParams.search, $stateParams.sortField, $stateParams.sortDir)
                    ...
                    .then(referenceData => {
                        ...
                        for (var name in referenceData) {
                            AdminDescription.getEntryConstructor()
                                .createArrayFromRest(referenceData[name], [references[name].targetField()], references[name].targetEntity()
                                    .name(), references[name].targetEntity()
                                    .identifier()
                                    .name())
                                .map(entry => scope.datastore.addEntry(references[name].targetEntity()
    ...
    <a class="btn btn-primary" ng-click="exportToCsv()">
