export default function maReferenceField(ReferenceRefresher) {
    return {
        ...
        link: function(scope) {
            ...
            const identifierName = field.targetEntity()
                .identifier()
                .name()
            ...
            if (!field.remoteComplete()) {
                ...
                let initialEntries = scope.datastore()
                    .getEntries(field.targetEntity()
                        .uniqueId + '_choices');
                if (scope.value) {
                    const isCurrentValueInInitialEntries = initialEntries.filter(e => e.identifierValue === scope.value)
                        .length > 0;
                ...
                const initialChoices = initialEntries.map(entry => ({
                    ...
                    label: entry.values[field.targetField()
                        .name()]
                }));
                    ...
                    return ReferenceRefresher.refresh(field, scope.value, search)
                        .then(function addCurrentChoice(results) {
                            if (!search && scope.value) {
                                const isCurrentValueInEntries = results.filter(e => e.value === scope.value)
                                    .length > 0;
                                if (!isCurrentValueInEntries) {
                                    ...
                                    results.unshift({
                                        ...
                                        label: currentEntry.values[field.targetField()
                                            .name()]
                                    });
                                }
                            }
