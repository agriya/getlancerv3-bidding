...
export default function maFileField(Upload) {
    return {
        ...
        link: {
            pre: function(scope) {
                var uploadInformation = scope.field()
                    .uploadInformation();
            ...
            post: function(scope, element) {
                ...
                scope.fileSelected = function(selectedFiles) {
                    ...
                    for (var file in selectedFiles) {
                        uploadParams = angular.copy(scope.field()
                            .uploadInformation());
                        ...
                        Upload.upload(uploadParams)
                            ...
                            .success(function(data, status, headers, config) {
                                ...
                                if (scope.apifilename) {
                                    var apiNames = Object.keys(scope.files)
                                        .map(function(fileindex) {
                                            return scope.files[fileindex].name;
                                        });
                                    scope.value = apiNames.join(',');
                                } else {
                                    scope.value = Object.keys(scope.files)
                                        .join(',');
                                }
                            })
