function getDetailLinkRouteName(field, entity) {
    if (entity.isReadOnly) {
        return entity.showView()
            .enabled ? 'show' : false;
    }
    if (field.detailLinkRoute() == 'edit' && entity.editionView()
        .enabled) {
        return 'edit';
    }
    return entity.showView()
    ...
    return {
        ...
        link: {
            pre: function(scope) {
                ...
                var referenceEntity = field.targetEntity()
                    .name();
                var relatedEntity = NgAdminConfiguration()
                    .getEntity(referenceEntity);
                var referenceId = scope.entry()
                    .values[field.name()];
