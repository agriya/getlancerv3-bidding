...
describe('directive: choice-field', function() {
    ...
    it("should contain a ui-select tag", function() {
        ...
        expect(uiSelect.classList.contains('ui-select-container'))
            .toBeTruthy();
    });
    it("should add any supplied attribute", function() {
        ...
        var element = $compile(directiveUsage)(scope);
        scope.$digest();
        expect(element.children()[0].getAttribute('disabled'))
            .toBeTruthy();
    });
    it('should allow to remove selected option only if not required', function() {
        function test(isRequired, expectedAllowClearValue) {
            ...
            var element = $compile(directiveUsage)(scope);
            scope.$digest();
            expect(element[0].querySelector('.ui-select-match')
                    .getAttribute('allow-clear'))
                .toEqual(expectedAllowClearValue.toString());
        }
        test(true, false);
        test(false, true);
    });
    it("should contain the choices as options", function() {
        ...
        var element = $compile(directiveUsage)(scope);
        scope.$digest();
        var uiSelect = angular.element(element.children()[0])
            .controller('uiSelect');
        expect(angular.toJson(uiSelect.items))
    ...
    it("should have the option with the bounded value selected", function() {
        ...
        var uiSelect = angular.element(element.children()[0])
            .controller('uiSelect');
        expect(angular.toJson(uiSelect.selected))
            .toEqual(JSON.stringify({
                label: 'baz',
                value: 'bazValue'
            }));
