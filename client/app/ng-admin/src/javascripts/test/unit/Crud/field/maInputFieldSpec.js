...
describe('directive: input-field', function() {
    ...
    angular.module('testapp_InputField', [])
        .directive('maInputField', directive);
    var $compile,
        scope,
    ...
    it("should contain an input tag", function() {
        ...
        expect(element.children()[0].nodeName)
            .toBe('INPUT');
        expect(element.children()[0].type)
            .toBe('text');
    });
    ...
    it("should add any supplied attribute", function() {
        ...
        var element = $compile(directiveUsage)(scope);
        scope.$digest();
        expect(element.children()[0].getAttribute('autocomplete'))
            .toEqual('off');
    });
    it("should use the field min and max attributes", function() {
        ...
        expect(input.getAttribute('min'))
            .toEqual('-2');
        expect(input.getAttribute('max'))
            .toEqual('2');
    });
    ...
    it('should set `step` attribute in scope if passed as element attribute', function() {
        ...
        var element = $compile(directiveUsage)(scope);
        scope.$digest();
        expect(element.find('input')[0].step)
            .toBe('any');
    });
});