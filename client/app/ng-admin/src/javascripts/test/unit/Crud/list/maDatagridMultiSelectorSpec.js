...
describe('directive: ma-datagrid-multi-selector', function() {
    ...
    beforeEach(inject(function(_$compile_, _$rootScope_) {
        $compile = _$compile_;
        scope = _$rootScope_;
        scope.entries = [new Entry('entity', {
            some: 'values'
        }, 'entity_1'), new Entry('entity', {
            some: 'values'
        }, 'entity_2'), new Entry('entity', {
            some: 'values'
        }, 'entity_3')];
        scope.selection = [new Entry('entity', {
            some: 'values'
        }, 'entity_2'), new Entry('entity', {
            some: 'values'
        }, 'entity_3')];
    ...
    it('checkbox should be indeterminate if entries does not correspond to selection', function() {
        ...
        scope.$digest();
        expect(element.children()[0].indeterminate)
            .toBe(true);
        expect(element.children()[0].checked)
            .toBe(false);
    });
    ...
    it('checkbox should become indeterminate once entries stop corresponding to selection', function() {
        ...
        expect(element.children()[0].checked)
            .toBe(true);
        scope.entries = scope.entries.concat(new Entry('entity', {
            some: 'value'
        }, 'entity_4'));
        scope.$digest();
        expect(element.children()[0].indeterminate)
