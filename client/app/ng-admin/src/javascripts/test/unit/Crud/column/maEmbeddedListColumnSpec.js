...
describe('directive: ma-embedded-list-column', function() {
    ...
    it('should contain a datagrid', function() {
        ...
        scope.$digest();
        expect(element.children()[0].nodeName)
            .toBe('MA-DATAGRID');
        const datagridScope = element.children()
            .eq(0)
            .scope();
        expect(datagridScope.entries.length)
            .toBe(2);
        expect(datagridScope.entries[0].values)
            .toEqual({
                num: 1,
                name: 'foo',
                dummy: 0
            });
        expect(datagridScope.entries[1].values)
            .toEqual({
                num: 2,
                name: 'bar',
                dummy: 1
            });
        expect(datagridScope.targetFields.length)
            .toBe(2);
        expect(datagridScope.targetFields[0].name())
            .toBe('num');
        expect(datagridScope.targetFields[1].name())
            .toBe('name');
    });
    it('should sort the list according to the sortField', function() {
        ...
        var element = $compile(directiveUsage)(scope);
        scope.$digest();
        const datagridScope = angular.element(element.children()[0])
            .scope();
        expect(datagridScope.entries.length)
            .toBe(2);
        expect(datagridScope.entries[0].values)
        ...
        expect(datagridScope.sortField)
            .toEqual('num');
        expect(datagridScope.sortDir)
            .toEqual('DESC');
    });
