...
describe('directive: json-field', function() {
    ...
    angular.module('testapp_JsonField', [])
        .directive('maJsonField', directive);
    var $compile,
        scope,
    ...
    it("should add any supplied attribute", function() {
        ...
        var element = $compile(directiveUsage)(scope);
        scope.$digest();
        expect(element.children()[0].placeholder)
            .toEqual('fill me!');
    });
    ...
    it("should be initialized with the stringified bounded value as textarea value", function() {
        ...
        expect(element.find('textarea')
                .val())
            .toBe(JSON.stringify(JSON.parse('["foo",{"bar":123}]'), null, 2));
    });
    it("should convert the JSON string into a JavaScript object value", function() {
        ...
        element.find('textarea')
            .val(JSON.stringify(JSON.parse('["foo",{"bar":456}]'), null, 2));
        ...
        expect(JSON.stringify(scope.value))
