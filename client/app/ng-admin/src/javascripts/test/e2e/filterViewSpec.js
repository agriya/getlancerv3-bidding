...
describe('List filter', function() {
    ...
    describe('layout', function() {
            ...
            $$('.filters .filter input')
                .then(function(inputs) {
                    expect(inputs.length)
                        .toBe(1); // Only text filter is pinned
        ...
        it('should display a filter button for adding new filters', function() {
            return $$('ma-view-actions button')
                .then(function(buttons) {
                    expect(buttons[0].getText())
                        .toBe(' Add filter ');
                });
        });
    });
    describe('filter button', function() {
        it('should display a list of filters when clicked', function() {
            element(by.css('ma-filter-button button'))
                .click();
            $$('ma-filter-button ul')
                .then(function(element) {
                    expect(element[0].getCssValue('display'))
                        .toBe('block');
                });
            $$('ma-filter-button ul li')
                .then(function(elements) {
                    expect(elements.length)
                        .toBe(2);
                    expect(elements[0].getText())
                        .toBe('Posted');
                    expect(elements[1].getText())
                        .toBe('Post');
                });
            hasToLoad = false;
        });
        it('should add a filter when filter name is clicked', function() {
            $$('ma-filter-button ul li:nth-child(1) a')
                .click();
            $$('.filters .filter input')
                .then(function(inputs) {
                    expect(inputs.length)
        ...
        it('should disappear if all filters were added', function() {
            element(by.css('ma-filter-button ul li:nth-child(1) a'))
                .click();
            $$('.filters .filter input')
                .then(function(inputs) {
                    expect(inputs.length)
                        .toBe(4); // Post is autocomplete, so it has 2 inputs
                });
            $$('ma-filter-button ul')
                .then(function(elements) {
                    expect(elements.length)
        ...
        it('should reappear once an unpinned filter is removed', function() {
            element(by.css('.filters .filter:nth-child(3) .remove_filter a'))
                .click();
            $$('ma-filter-button ul')
                .then(function(elements) {
                    expect(elements.length)
    ...
    describe('text filter', function() {
        it('should filter globally', function() {
            ...
            $$('.filters .filter:nth-child(1) input')
                .sendKeys('rabbit')
                .then(function() {
                ...
                .then(function(tdElements) {
                    expect(tdElements.length)
                        .toBe(1);
                    expect(tdElements[0].getText())
                        .toBe('White Rabbit: it was indeed: she was out of the gr...');
                });
            hasToLoad = false;
        });
        it('should update the pagination total', function() {
            $$('ma-datagrid-pagination .total')
                .then(function(totalElements) {
                    expect(totalElements[0].getText())
                        .toBe('1 - 1 of 1');
                });
            hasToLoad = false;
        });
    ...
    describe('reference filter', function() {
        it('should filter on reference', function() {
            $$('ma-filter-button button')
                .click();
            $$('ma-filter-button ul li:nth-child(2) a')
                .click();
            // Filter on post_id '3' (Perspiciatis adipisci vero qui ipsam iure porro)
            $$('.filters .ui-select-placeholder')
                .click();
            $$('#ui-select-choices-row-0-2')
                .click();
            ...
            $$('.grid tr td:nth-child(4)')
                .then(function(tdElements) {
                    expect(tdElements.length)
                        .toBe(2);
                    expect(tdElements[0].getText())
                        .toBe('I\'d been the whiting,\' said the Hatter, it woke up...');
                    expect(tdElements[1].getText())
                        .toBe('I\'m not Ada,\' she said, \'and see whether it\'s mark...');
                });
            hasToLoad = false;
        });
        it('should update the pagination total', function() {
    ...
    describe('filter choice conflict', function() {
        ...
        var addFilter = function(name) {
            return function() {
                return element(by.css('ma-filter-button'))
                    .click()
                    .then(function() {
        ...
        var removeFilter = function(name) {
            return function() {
                return element(by.css('.filter.' + name + ' .remove_filter'))
                    .click();
            };
        };
        var getChoices = function(name) {
            return function() {
                return element(by.css('.filter.' + name))
                    .click()
                    .then(function() {
                        return element.all(by.css('.filter.' + name + ' .ui-select-choices-row'))
                            .getText();
                    });
            };
        };
        it('should not mix filter choices when adding two filter and then deleting the first', function() {
            addFilter('Category')()
                ...
                .then(function(categoryChoices) {
                    return expect(categoryChoices)
                        .toEqual(['Tech', 'Lifestyle']);
                })
                .then(addFilter('Subcategory'))
                .then(getChoices('subcategory'))
                .then(function(subCategoryChoices) {
                    return expect(subCategoryChoices)
                        .toEqual(['Computers', 'Gadgets', 'Travel', 'Fitness']);
                })
    ...
    describe('interaction with pagination', function() {
        it('should reset page number', function() {
            ...
            $$('.filters .filter:nth-child(1) input')
                ...
                .then(function() {
                    return $$('ma-datagrid-pagination li:nth-child(3) a')
                        .click();
                })
                .then(function() {
                    return $$('ma-datagrid-pagination .total');
                ...
                .then(function() {
                    return $$('.filters .filter:nth-child(1) input')
                        .clear();
                })
                .then(function() {
