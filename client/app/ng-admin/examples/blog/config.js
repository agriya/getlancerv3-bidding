...
(function() {
        ...
        RestangularProvider.addResponseInterceptor(function(data, operation, what, url, response) {
            if (operation === "getList") {
                ...
                if (headers['content-range']) {
                    response.totalCount = headers['content-range'].split('/')
                        .pop();
                }
            }
            return data;
    ...
    app.config(['NgAdminConfigurationProvider', function(NgAdminConfigurationProvider) {
        ...
        post.listView()
            ...
            .fields([
                nga.field('id')
                .label('id'), // The default displayed name is the camelCase field name. label() overrides id
            ...
            .exportFields([
                post.listView()
                .fields(), // fields() without arguments returns the list of fields. That way you can reuse fields from another view to avoid repetition
        ...
        tag.listView()
            .prepare(['Restangular', 'entries', function(Restangular, entries) {
                // fetch the number of posts for each tag
                return Restangular.allUrl('posts')
                    .getList()
        ...
        return {
            ...
            link: function(scope) {
                scope.displayPost = function() {
                    $location.path('/posts/show/' + scope.entry()
                        .values.post_id); // jshint ignore:line
                };
            }
        };
    }]);
    app.directive('sendEmail', ['$location', function($location) {
        return {
            ...
            link: function(scope) {
                scope.send = function() {
                    $location.path('/sendPost/' + scope.post()
                        .values.id);
                };
            }
        };
    ...
    sendPostController.prototype.sendEmail = function() {
        if (this.email) {
            ...
            this.notification.log('Email is undefined', {
